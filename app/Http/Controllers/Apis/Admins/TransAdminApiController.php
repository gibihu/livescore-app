    ...
    public function update(Request $request)
    {
        try {
            ...
            if ($status && $tran_id) {
                switch ($status) {
                    case 'approved':
                        $tranDB = Transaction::with('user')->where('id', $tran_id)->first();
                        $customer = User::find($tranDB->user_id);
                        if ($tranDB && $customer) {
                            if ($tranDB->status == Transaction::STATUS_APPROVED || $tranDB->status == Transaction::STATUS_REJECTED) {
                                throw new Exception('ไม่สามารถบันทึกซ้ำได้');
                            }
                            ...
                            if ($wallet) {
                                ...
                                $tranDB->user->rank->update([
                                    'score' => $tranDB->user->rank->score + ($tranDB->points * 0.1),
                                ]);
